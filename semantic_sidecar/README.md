...
## Mallas por instancia o escena (F6)

- `POST /mallas` `{escena, ambito:"instancia"|"escena", id_instancia:int|null, formato:"glb"|"3mf", archivo_b64, metadatos?}` guarda `artifacts/mallas/<escena>/<id_instancia>.<formato>` o `artifacts/mallas/<escena>/escena.<formato>` (+ JSON). La malla la calcula el visor (TSDF + surface nets o marching tetrahedra); el sidecar sólo la persiste. `glb_b64` continúa aceptado para clientes GLB anteriores. Open3D (`ScalableTSDFVolume`, Poisson) queda como backend opcional futuro: `pip install open3d` arrastra dependencias de visualización (plotly, dash) y no se instala aquí.

...
| 2g Mesh (F6) | Isolated instance → orbit cameras → depth (mean or 2DGS median) + colour → TSDF fusion with carving → surface nets → largest component → GLB (normals + vertex colours) | `shared/tsdf.js` in `tsdf-worker.js`, `shared/glb.js`, HUD *Malla*, sidecar `/mallas` → `artifacts/mallas/` |

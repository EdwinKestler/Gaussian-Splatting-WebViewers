...
flowchart TD
  ...
  C --> H["Asociación entre vistas<br/>grafo recíproco: superpuntos + soporte gaussiano"]
  G --> H
...
> **Actualización vigente (3 sep 2026).** La asociación entre vistas usa un grafo de mejores coincidencias recíprocas con contención de superpuntos y solapamiento gaussiano directo (20 máscaras del reloj → 13 instancias y 7 fusiones, sin verdad terreno). El perfil reproducible en una RTX 3090 Ti mide color/profundidad/K-buffer y grafo; el refresco posterior a la reparación ejecuta la malla GLB completa en 1,126 s. F6 añade validación/reparación, remallado por marching tetrahedra y 3MF en milímetros; la aceptación sintética obtiene 0 bordes abiertos y 0 aristas no-manifold a 80 mm, mientras que el 3MF 96³ del reloj se bloquea por geometría residual en vez de exportarse sin comprobar. Hay 122 pruebas unitarias y 23 e2e por defecto más una e2e opcional de modelos. El bloque largo siguiente conserva la bitácora del primer incremento y sus medidas históricas; no debe usarse como estado actual.

> **Estado (3 sep 2026): F0–F7 implementadas en su primer incremento** en la rama `claude/gaussian-splat-plan-f0saqs` (107 pruebas unitarias, 22 e2e + 1 opcional con modelos en el navegador; resumen de método y resultados en el reporte técnico §6, flujo en `docs/figures/segmentation-pipeline.png`). Detalle por fase: F0, F1, F2, F3 (primer incremento) y F4 implementados: GaussForge vendorizado (probado con el CDN bloqueado), PLY 2DGS, `artifacts/`, `npm test` (84 pruebas) y `npm run test:e2e` (18 pruebas, Chromium + SwiftShader, render *offscreen*), búfer de etiquetas + tabla de instancias + modos ID/profundidad/normal + `renderOffscreen()`/`pick()` en `gpu-renderer.js`, panel **Instancias** en el HUD y escena sintética de dos esferas con profundidad verificada al 0,0 %. El modo profundidad de `renderOffscreen` sigue siendo la media ponderada por alfa; la mediana 2DGS exacta la devuelve `renderContributions().medianDepth` (K-buffer). Ver `docs/testing.md`. **F2:** `shared/graph.js` + `graph-worker.js` (kNN k=10 con rejilla hash ordenada por celda, pesos Mahalanobis simétricos × color SH0, umbral, componentes conexas → superpuntos, difusión por mayoría ponderada), panel **Grupos**, modo de color «Grupos» con búfer `groups` propio (no consume la tabla de 4096 instancias) y clic → grupo a instancia; dos esferas sintéticas → exactamente 2 grupos; 1 M gaussianas ≈ 4–5 s en un contenedor de 4 vCPU sin GPU (`scripts/bench-graph.mjs`), pendiente de medir en portátil. No hecho de F2: segunda pasada tipo Felzenszwalb (nivel «parte»). **F3:** `contrib-pass.js` (K-buffer exacto: lista ordenada por profundidad dibujada en trozos que se dividen si algún píxel supera K = 16, T por píxel persistente, masa α·T por etiqueta en punto fijo con lotes de columnas ≤ 64 MB), `shared/lift.js` (argmax FlashSplat con sesgo de fondo, asociación tipo Gaga por contención de histogramas de superpuntos, exportación `instancias.json`/`etiquetas.u32`), panel **Segmentación** (fuentes: etiquetas actuales, sidecar cajas Grok → elipses, sidecar SAM vía `SAM_BACKEND`), sidecar `/segment` y `/segmentaciones`. Aceptación sintética: 6 vistas con ids permutados → 2 instancias, IoU 3D 0,9985/1,0 y 1,0/1,0 tras difusión. Pendiente de F3: SAM 2 en el navegador (ONNX), verificación sobre `model.splat` con máscaras reales (misma silla = una instancia), embeddings CLIP. **F4:** render aislado por instancia (`shared/naming.js` encuadra la caja de la instancia y aísla sus gaussianas), sidecar `/name` (Grok VQA con esquema JSON en español; `NAME_BACKEND=mock` para pruebas sin clave), panel Instancias con `nombre_es · categoría · confianza`, búsqueda por texto sin acentos con selección por Intro, tarjetas Imagine ligadas a `id_instancia`, exportación con nombres. **Modelos en el navegador (SAM 2 y CLIP, 3 sep 2026):** `gaussian_splatting_webgpu/ml-browser.js` ejecuta SAM 2.1 hiera-tiny (fp16) y CLIP ViT-B/32 (q8) con transformers.js 4.2 + ONNX Runtime Web (WebGPU en GPU real, WASM en su defecto); `scripts/download-ml-models.sh` vendoriza biblioteca y pesos (≈ 240 MB, `vendor/ml/` ignorado) para funcionar sin CDN ni Hub. Fuente «SAM 2 (navegador)» del panel Segmentación: indicaciones = centroides proyectados de los mayores superpuntos visibles (comprobados con el pase ID), máscaras casi duplicadas fusionadas por IoU ≥ 0,8; en la escena sintética 2 vistas → exactamente 2 instancias de 2000 gaussianas (2 fusiones, codificación ≈ 21 s por vista en WASM bajo SwiftShader, decodificación ≈ 0,4 s por indicación). Botón *Embeddings CLIP* + casilla *Búsqueda semántica*: 512 d por recorte aislado (≈ 0,5 s por instancia en WASM), «an orange ball» → esfera naranja y «a blue sphere» → esfera turquesa; `instancias.json` añade `embedding_clip` (redondeado a 1e-4) y un bloque `embeddings {modelo, dimension}`. Prueba opcional `ML_E2E=1 npx playwright test tests/e2e/f4-ml-browser.spec.mjs`. **Máscaras reales sobre el despertador (cajas de Grok):** 4 vistas → 5 instancias con 0 fusiones (una caja gruesa por vista que cubre casi todo el reloj; las elipses no se asocian entre vistas a 90° con superpuntos tan pequeños) y el nombrado bautiza los fragmentos como «taburete rojo», «sofá rojo»…: las cajas de Grok no sirven como máscaras reales, de ahí SAM 2 en el navegador. **Máscaras reales sobre el despertador (SAM 2 en el navegador, 4 vistas, 8 indicaciones por vista):** SAM sí devuelve partes coherentes (cuerpo, esfera, manecillas, campana, patas, botón; puntuaciones 0,73–0,97) y la fusión de duplicados descarta 12 de 31 máscaras; el levantamiento da 17 instancias con sólo 2 fusiones entre vistas (la mayor, 70 158 gaussianas, nombrada «reloj analógico» 0,72), es decir, la misma parte vista desde ángulos distintos suele quedar como instancia aparte. Coste bajo SwiftShader/WASM: ≈ 21 s de codificación SAM por vista + ≈ 35 s de K-buffer por vista (279 s en total). Las búsquedas CLIP («a bell», «a clock face») ordenan las 17 instancias con similitudes muy planas (0,20–0,24): los recortes de fragmentos no son discriminativos. Siguiente paso para F3 sobre escenas reales: asociar entre vistas por solapamiento 3D tras el levantamiento (no sólo por contención de histogramas de superpuntos, que con 113 k superpuntos es demasiado fino) y subir el umbral de F2 para superpuntos más grandes. **Revisión de lo aplazado por conectividad (3 sep 2026):** en este entorno el Chromium sin cabeza no llega al CDN ni al Hub, por eso GaussForge y los modelos ONNX se vendorizan; la ruta sin vendorizar de `ml-browser.js` (jsDelivr + Hugging Face) queda verificada por URL a través del proxy (transformers.js 4.2.0, GaussForge 0.6.0, SAM 2.1 tiny/small fp16, SlimSAM fp32/q8, CLIP q8/fp16: 13/13 responden 200) aunque no se ha podido ejecutar en el navegador aquí; el nombrado real con Grok (12,6 s) y la tarjeta Imagine real (7,7 s, `img_output/`) funcionan a través del sidecar. Sin resolver por licencia, no por red: un backend SAM en el sidecar (`SAM_BACKEND`) y ArtiFixer. **F5 (primer incremento, 3 sep 2026):** `shared/edit-ops.js` (registro `ops.jsonl` con `asignar`/`transformar`/`borrar`/`restaurar`/`duplicar`/`fusionar`/`renombrar`, reproducción determinista, deshacer/rehacer por reproducción, horneado de transformaciones rígidas + escala y `origen` como índice invariante), `shared/export-io.js` (PLY 3DGS binario con `instance_id`/`class_id`/`confidence` como columnas float, `.splat` SH0; SPZ y PLY comprimido vía GaussForge vendorizado desde ese PLY), `readPlyColumns` en `splat-io.js` y el worker devuelven las instancias al recargar un PLY exportado, panel **Edición** (rectángulo, pincel, esfera 3D y superpunto con añadir/quitar sobre el búfer ID; mover/rotar/escalar, duplicar, borrar, fusionar, renombrar, Ctrl+Z), sidecar `/exportaciones` → `artifacts/exportaciones/<escena>/` y `ops.jsonl` + `etiquetas_base.u32` junto a `instancias.json`. Aceptación cumplida en `tests/e2e/f5-edit.spec.mjs`: exportar una instancia y recargarla muestra sólo ese objeto (decodificado por GaussForge, instancia restaurada desde `instance_id`), reproducir `ops.jsonl` sobre una escena nueva da la misma huella. **H3 sobre el despertador:** selección por esfera (4 986 gaussianas) → instancia, mover + rotar 30° en 7 ms, SPZ de la instancia (70 KB) en 57 ms, SPZ de la escena completa (262 144 gaussianas, 3,7 MB) en 1,7 s, deshacer en 0,3 s. No hecho de F5: lazo, dividir por superpuntos como op propia (se cubre con selección «superpunto» + nueva instancia), horneado exacto de escalas no uniformes (se aproxima por ejes), relleno de agujeros tras borrar (fuera de alcance v1). **F6 (primer incremento, 3 sep 2026):** `shared/tsdf.js` (cámaras en esfera de Fibonacci alrededor de la instancia aislada, fusión TSDF con tallado del espacio vacío, extracción por *surface nets*, mayor componente conexa, estadísticas) en `shared/tsdf-worker.js`, `shared/glb.js` (glTF 2.0 binario con normales y color por vértice), panel **Malla** y botón «Malla» por instancia (profundidad media rápida o mediana 2DGS por K-buffer), sidecar `/mallas` → `artifacts/mallas/<escena>/<id>.glb` y campo `malla` en `instancias.json`. Se implementa en JS en lugar de Open3D en el sidecar (el plan lo dejaba para «más adelante»): funciona sin red y sin dependencias; `pip install open3d` arrastra plotly/dash y queda como backend opcional futuro. Aceptación (`tests/e2e/f6-mesh.spec.mjs`): esfera sintética → GLB válido (pygltflib), una componente, radio medio 0,535–0,545 frente a 0,5 (+7–9 %: la profundidad media sobreestima el radio por la extensión de los splats; margen documentado ±12 %), color por vértice correcto; la distancia de Chamfer contra malla de referencia sigue pendiente (no hay malla de referencia del despertador). **H4 sobre el despertador:** instancia con las 262 144 gaussianas, 24 vistas de 256×160, 96³ vóxeles → 12 425 vértices / 24 866 triángulos, GLB de 0,75 MB reconocible (cuerpo, esfera, campanas, patas y asa con color); tiempos bajo SwiftShader: render 374 s (≈ 7,8 s por vista entre profundidad y color), fusión 0,8 s, extracción 0,1 s; en GPU real el render debería bajar a segundos. No hecho de F6: vista previa de la malla dentro del visor, decimación, vía Poisson y Open3D. **F7 (3 sep 2026):** `docs/pipeline.md` reescrito (flujo F0–F6, recorrido del HUD, disposición de `artifacts/`, ficheros de datos, endpoints), figura `docs/figures/segmentation-pipeline.{mmd,svg,png}` generada con `scripts/render-mermaid.mjs` (Mermaid local + Chromium, sin red), reporte técnico con la nueva §6 (etapas, resultados medidos, desviaciones), tablas de software/métodos/licencias al día (transformers.js, ONNX Runtime Web, SAM 2.1, CLIP, Playwright, Pillow, redis, Mermaid; FlashSplat, Gaga, LUDVIG, 2DGS, KinectFusion, surface nets, SuperSplat) y referencias [22]–[33], README reordenado por fases, esquema `shared/schemas/instancias.schema.json` + `shared/schemas.js` (validación en la exportación del visor y `scripts/validate-instancias.mjs`; cumple §3.3). El PDF del reporte se regenera con `scripts/render-report-pdf.mjs` (marked + Chromium, sin pandoc). Pendiente de F7: medir en GPU real y actualizar las tablas de tiempos. **Prueba con `splats/alarm_clock_generated.splat` (262 144 gaussianas, SH0):** F2 en 3,8 s (113 154 superpuntos con umbral 0,3; ajustar el umbral para escenas reales), F3 funciona de extremo a extremo pero el K-buffer cuesta ≈ 35 s por vista a 640×400 bajo SwiftShader (≈ 670 trozos: el splat apila ~100 fragmentos por píxel); se añadieron un umbral de alfa (0,02), K = 24 con entradas de 12 bytes y trozos iniciales pequeños; en GPU real se espera del orden de segundos por vista, pendiente de medir.
...
| **F3 Levantar** | K-buffer WebGPU; asignación FlashSplat; SAM 2 en navegador; asociación recíproca por superpuntos + soporte gaussiano; `instancias.json` + `etiquetas.u32` | Máscara sintética → IoU 3D > 0,9; la escena real registra máscaras, fusiones e instancias reproducibles | 3 semanas |
...
| **F6 Malla** | Órbita + profundidad/color → TSDF JS → GLB; vía de impresión por marching tetrahedra, reparación, validación y 3MF en mm | Esfera: GLB dentro del margen; 3MF con 0 bordes abiertos/no-manifold, escala exacta y paquete OPC válido | 2 semanas |
...
- **H4 (fin F6):** "Malla" descarga GLB o un 3MF cerrado a la dimensión solicitada en milímetros.
...
```text
gaussian_splatting_webgpu/   visor: gpu-renderer, contrib-pass (K-buffer), ml-browser (SAM 2.1 + CLIP), parse-worker, HUD (main.js, index.html)
shared/                      splat-io, graph(+worker), lift, naming, edit-ops, export-io, tsdf(+worker), mesh-ops, glb, three-mf, schemas, synthetic, instances
semantic_sidecar/            server.py (/analyze, /card, /segment, /name, /segmentaciones, /exportaciones, /mallas)
scripts/                     vendor/model setup, benchmark, perfil WebGPU, validación, Mermaid/PDF y check-source
artifacts/                   segmentaciones/, exportaciones/, mallas/, test-results/ (gitignored salvo README)
vendor/                      gaussforge/ versionado en git (Apache-2.0, NOTICE.md); ml/ gitignored (transformers.js + ONNX Runtime + SAM 2.1 + CLIP, ≈ 240 MB)
docs/                        este plan, pipeline.md, reporte técnico (+ figures/), testing.md, webgpu-chrome.md
tests/                       unit/ (node --test) y e2e/ (Playwright, Chromium WebGPU)

Lo previsto como `select.js` / `segments-io` / `mesh.py` / `embed.py` quedó en `shared/edit-ops.js`, `shared/lift.js`, `shared/tsdf.js` y `ml-browser.js` (todo en el navegador; el sidecar sólo nombra y persiste).
```

...
**Segmentation, editing and meshing (plan F0–F6, no per-scene training)** — data flow in [docs/figures/segmentation-pipeline.png](docs/figures/segmentation-pipeline.png), operator walkthrough and `artifacts/` layout in [docs/pipeline.md](docs/pipeline.md), method and measured results in the [technical report](docs/open-vocab-3dgs-imagine-pipeline-paper.md) §6, per-phase status in the [plan](docs/plan-segmentacion-edicion-3dgs.md).
...
- **Mask lifting (plan F3):** `gaussian_splatting_webgpu/contrib-pass.js` accumulates per-Gaussian α·T mass per 2D mask label and resolves the exact 2DGS median depth. `shared/lift.js` applies FlashSplat assignment, then an order-independent reciprocal-best graph combines superpoint containment with direct Gaussian overlap while prohibiting two masks from the same view in one instance. The HUD exports `instancias.json` / `etiquetas.u32`; `/segment` and `/segmentaciones` provide optional sidecar masks/persistence
...
- **In-browser models (plan F3/F4, optional):** `gaussian_splatting_webgpu/ml-browser.js` runs **SAM 2.1 hiera-tiny** and **CLIP ViT-B/32** with transformers.js + ONNX Runtime Web (WebGPU on a real GPU, WASM otherwise). Mask source *SAM 2 (navegador)* prompts SAM with the projected centroids of the largest visible F2 superpoints and lifts the masks like any other source; *Embeddings CLIP* embeds each isolated instance crop and *Búsqueda semántica (CLIP en el navegador)* ranks instances by cosine similarity to the query (Enter selects the best); the export gains `embedding_clip` per instance and an `embeddings` block. Run `scripts/download-ml-models.sh` once to vendor the library and weights (~240 MB, gitignored `vendor/ml/`) so nothing is fetched from the CDN or the Hub; without it the browser loads them from jsDelivr/Hugging Face and keeps them in its cache. No API key is involved
- **Editing and export (plan F5):** HUD panel **Edición**. Selection tools on the ID buffer (*Rectángulo*, *Pincel*, *Esfera 3D*, *Superpunto*, modes *Añadir*/*Quitar*) paint a live selection that becomes a new instance, joins the selected one or goes back to fondo. Per instance: move/rotate/scale (rigid transform on the GPU instance table), *Duplicar*, *Borrar*/*Restaurar*, *Fusionar con #*, *Renombrar*, with *Deshacer*/*Rehacer* (Ctrl+Z). Every change is an op in `ops.jsonl` (`shared/edit-ops.js`: replaying the log over the base labels reproduces the state). *Exportar* bakes the transforms and writes the selected instance or the visible scene as 3DGS **PLY** with `instance_id`/`class_id`/`confidence` columns (reloading it restores the instances), **.splat** (SH0), or **SPZ** / compressed PLY through the vendored GaussForge (`shared/export-io.js`); files download and, with the sidecar, land in `artifacts/exportaciones/<escena>/` via `/exportaciones`; *Guardar ops.jsonl* stores the log with `etiquetas_base.u32` under `artifacts/segmentaciones/`
- **Mesh and print export (plan F6):** HUD panel **Malla** exports either the selected instance or the complete visible scene; the per-instance *Malla* button remains a shortcut for one object. The viewer frames the target, renders a Fibonacci orbit, and fuses mean or 2DGS-median depth into a TSDF. Whole-scene export retains every disconnected visible component and writes `*_escena.glb|3mf`. GLB keeps the fast surface-nets mesh and vertex colours. The 3MF path uses marching tetrahedra, weld/removal/winding/small-hole repair, validates boundary and non-manifold edges, scales the longest dimension to the requested millimetres, places the part on z=0, and writes a 3MF OPC package (`shared/mesh-ops.js`, `shared/three-mf.js`). A failed printability gate blocks 3MF. This does not validate wall thickness, self-intersections, supports, slicing, or a physical print; vanilla 3DGS remains noisy, so use 2DGS/GOF input for production work
...
npm run check        # syntax-check first-party JS/Python and parse JSON
npm test             # 122 Node unit tests, including topology repair and 3MF packaging
npm run test:e2e     # 23 default Playwright tests on Chromium WebGPU + one opt-in model test
npm run profile:webgpu  # reproducible adapter/render/graph/mesh profile; see docs/performance.md
...
Details, GPU flags (`WEBGPU_ARGS`) and the SwiftShader canvas caveat: [docs/testing.md](docs/testing.md). Generated outputs go under `artifacts/` (gitignored; layout in [docs/pipeline.md](docs/pipeline.md)); `node scripts/validate-instancias.mjs <instancias.json>` checks an export against `shared/schemas/instancias.schema.json`.

...
- **Mesh per instance (plan F6):** HUD panel **Malla** and the *Malla* button on each instance. The instance is isolated and orbited by *Vistas* cameras (Fibonacci sphere) rendering depth (alpha-weighted mean, or the exact 2DGS median from the K-buffer) plus colour; `shared/tsdf.js` fuses them into a truncated signed distance volume with empty-space carving and extracts the surface with naive surface nets (largest component kept) in a worker; `shared/glb.js` writes a glTF 2.0 binary with normals and vertex colours. The GLB downloads and, with the sidecar, is saved to `artifacts/mallas/<escena>/<id_instancia>.glb` via `/mallas`; `instancias.json` gains `malla`. Vanilla 3DGS gives a noisy surface (the HUD says so): use a 2DGS/GOF PLY for production meshes
